...
def doit(mode):
    ...
    policy = GaussianMLPPolicy(env.spec, hidden_sizes=(125,50,25))
    ...
    return results,rewards_per_episode


results = {'vanilla':[]}#, 'vanilla+z':[], 'linbaseline':[], 'linbaseline+z':[]}
for i in range(3):
    print(i)
    for key in results:
        results[key].append(doit(mode=key))
...
# for key in results:
#     plt.figure()
#     plt.plot(np.array(results[key]).T)
